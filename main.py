...
if __name__ == '__main__':
    ...
    if args.function.lower() == 'detect_faces':
        ...
        specific_parser.add_argument('-v', '--video_path', type=str, help="Video Path where the video to be analyzed can be found at, starting at project root folder")
        specific_parser.add_argument('-d', '--detector', type=str, help="Detector Model, default is scrfd. Currently available Detectors: ".format(DETECTORS), default="scrfd")
        specific_parser.add_argument('-c', '--classifier', type=str, help="Classification Model, default is one self-implemented classifier, that first embeds the patch and then measures the similarity. Currently available Classifiers (except meanshift): ".format(CLASSIFIERS), default="online")
        filter_args = ["video_path", "detector", "classifier"]
    elif args.function.lower() == 'offline_pipeline':
        specific_parser.add_argument('-v', '--video_path', type=str, help="Video Path where the video to be analyzed can be found at, starting at project root folder")
        specific_parser.add_argument('-d', '--detector', type=str, help="Detector Model, default is scrfd. Currently available Detectors: ".format(DETECTORS), default="scrfd")
        specific_parser.add_argument('-c', '--classifier', type=str, help="Classification Model, default is meanshift. Currently available Classifiers: ".format(CLASSIFIERS), default="meanshift")
        filter_args = ["video_path", "detector", "classifier"]
    else:
