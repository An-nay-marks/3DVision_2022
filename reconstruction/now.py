...
class NoWDataset(Dataset):
    def __init__(self, split, train_ratio, crop_size=224, scale=1.6, dist_path=None):
        ...
        base_dir = f'{ROOT_DIR}/data/NoW_Dataset/final_release_version'
        self.image_dir = os.path.join(base_dir, 'iphone_pictures')
        self.bbox_dir = os.path.join(base_dir, 'detected_face')
        self.scan_dir = os.path.join(base_dir, 'scans')
        self.lmk_dir = os.path.join(base_dir, 'scans_lmks_onlypp')

        with open(os.path.join(base_dir, 'imagepathsvalidation.txt')) as f:
